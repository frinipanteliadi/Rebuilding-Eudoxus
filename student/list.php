...
<html>
<head>
    <title>Εύδοξος - Λίστα Συγγραμμάτων</title>
...
<body>
    ...
    <h1 style="text-align: center; color: white; font-family: Verdana;">Τα συγγράμματά μου</h1>
    <div class="container" style="background-color: white;">
        <?php
            $query = "SELECT * FROM Student_has_Book WHERE Student_id=".$_SESSION['id'];
            $result = $conn -> query($query);
            while ($row = $result->fetch_assoc()) {
                $query2 =  "SELECT * FROM Book WHERE id=".$row['Book_id'];
                $result2 = $conn -> query($query2);
                $book = $result2->fetch_assoc();
                echo "<div class=\"container\">
                            <h1>".$book['name']."</h1>
                            <p>Συγγραφέας: ".$book['author']."</p>
                            <p>Εκδότης: ".$book['publication']."</p>
                            <p>Εξάμηνο: ".$book['semester']."</p>
                            </div>";
            }
        ?>
    </div>
